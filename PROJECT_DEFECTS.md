...
## جولة التسجيل الذاتي والانطلاق (2026-08-18) — Self-serve onboarding

كانت دورة الحساب تنتهي عند شاشة الدخول: مستخدم جديد يسجّل حسابه ثم يعلق في
حلقة دخول/خروج بلا أي منشأة (لا واجهة إنشاء منظمة ولا مسار تسجيل ذاتي،
و`create_organization` ملغى من أدوار المتصفح منذ الترحيل 0056). أُغلق هذا
العائق بالكامل:

- **الترحيل 0061** (`20260817170000_0061_self_serve_onboarding.sql`): إعادة منح
  `create_organization(text, text)` لـ`authenticated` فقط — أي مستخدم مسجَّل
  ينشئ منشأته ويصبح مالكها؛ `anon` يبقى بلا أي منح (0059). قابل للعكس بترحيل
  لاحق، لا يمس البيانات.
- **`/signup`** (صفحة إنشاء حساب بالبريد + كلمة مرور، مع تعريب كامل لأخطاء
  Supabase Auth ومسار تأكيد البريد).
- **`/forgot-password`** (استعادة كلمة المرور برابط بريدي قياسي).
- **شاشة «أنشئ منشأتك»** (`OnboardingPage`) بعد أول دخول لمستخدم بلا عضوية —
  بدل الطرد إلى شاشة الدخول؛ مع زر تسجيل خروج للحساب العالق.
- **عقود الاختبار:** `rls_isolation.test.sql` (المصادَق عليه يستطيع التنفيذ،
  المجهول لا يستطيع) و`commercial_invariants.test.sql` (الإنشاء الذاتي يُنجح
  المستخدم الجديد ويجعله OWNER مع بقاء مسار الدور المميز) — 590 تأكيداً مخططة
  عبر 16 ملفاً دون تغيير.
- الوثائق: `PRODUCT_GAPS.md` (جديد — ما أُنجز وما يُوصى به للانطلاق)، وتحديث
  `PRODUCT_SPEC.md`/`PROJECT_STATUS.md`/`OPERATIONS.md §9`/`README.md`/
  `PROJECT_OVERVIEW.md`/`AGENT_HANDOFF.md` + مسارا `/signup` و`/forgot-password`
  في فحص الإنتاج (smoke).

### الأدلة (شُغّلت في هذه الجلسة)
- الواجهة: **72 ملفاً / 542 اختباراً كلها ناجحة** · typecheck 0 أخطاء ·
  lint 0/0 (246 ملفاً) · build ✅ · smoke إنتاجي ✅ (يشمل المسارين الجديدين) ·
  `npm audit` 0 ✅
- قاعدة البيانات (فحص أصلي تكميلي على PostgreSQL 18.3 — PGlite، خارج
  المستودع): **61 ترحيلاً أُعيدت من قاعدة فارغة + 16 ملف pgTAP (590 تأكيداً)
  PASSED** ✅ — مع استبدال قيد استبعاد `btree_gist` الوحيد بمشغّل مكافئ في
  نسخة الفحص فقط؛ السلطة المرجعية CI الرسمي بمكدس Supabase الكامل.
- القرار التشغيلي المتبقي للمالك: تفعيل `enable_signup` في مشروع Supabase
  الإنتاجي، ثم `supabase db push` بعد نسخة احتياطية (OPERATIONS.md §9).
