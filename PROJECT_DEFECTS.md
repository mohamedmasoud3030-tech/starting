...
>   في `docs/archive/audits/FULL_PROJECT_AUDIT.md` تحت «مخاطر محتملة».
...
| D2 | Medium | وضع العرض العام نائم في مخطط الإنتاج (شرط اسم المنظمة) | FIXED ✅ في المخطط (الترحيل 0059) · تطبيق الإنتاج NEEDS_OWNER |
| D3 | Medium | قوائم بلا ترقيم صفحات — اقتطاع صامت عند 1000 صف | FIXED ✅ (مناسبات/عملاء/كتالوج بترقيم حقيقي) · المشتريات: تنبيه سقف قائم |
...
| D20 | Low | لا سياسة احتفاظ لسجل التدقيق | FIXED ✅ (الترحيل 0098: purge مملوك للمالك + سياسة 24 شهراً موثقة) |
| D21 | Medium | ترقيم صفحات حقيقي للقوائم (متابعة D3) | FIXED ✅ (مناسبات + عملاء + كتالوج: صفحات حقيقية؛ المشتريات تبقى بتحذير السقف) |
...
- **الوضع (مُسوّى 2026-09-09):** المخطط أصبح نظيفاً — الترحيل
  `20260817160000_0059_remove_public_demo_mode.sql` يحذف دور `public_demo_admin`
  ومنحه ودواله وأي بقايا للمسار التجريبي، ويتحقق ذلك من قاعدة مُعاد تشغيلها ومن
  سلسلة pgTAP. **المتبقي تشغيلي فقط ويحتاج قرار المالك:** تطبيق 0059 على قاعدة
  الإنتاج الفعلية، وقرار نهائي حول انتهاء فترة العرض التجريبي — لا يُنفَّذ تطبيق
  على الإنتاج دون موافقة المالك.
...
- **الإصلاح (مكتمل 2026-09-09):** ترقيم صفحات حقيقي (`.range()` لكل صفحة + عدّاد
  دقيق + ترتيب حاسم `order by id` ثانوي) لثلاث أسطح: المناسبات والعملاء والكتالوج
  (تفاصيل D21) — لم تعد هذه القوائم خاضعة لسقف 1000 صف إطلاقاً. أسطح المشتريات
  تحتفظ بعدّاد دقيق وتنبيه عربي صريح عند بلوغ السقف، والفلاتر المحلية تعمل ضمن
  بيانات الصفحة المعروضة.
- **التحقق:** اختبارات وحدة للمنطق + عقد `useEventsPage` للصفحات والحدود
  (`eventsPage.pagination.test.tsx`) + اختبارات `PaginationBar`.
...
### D17 — منطقة زمنية لإدخال المواعيد — Low — FIXED ✅
- **السلوك السابق:** `datetime-local` يُفسَّر بمنطقة جهاز المشغل؛ مشغل خارج +04
  يسجّل المواعيد منزاحة.
- **الإصلاح:** تثبيت `Asia/Muscat` في كل حدود الإدخال — تحقق 2026-09-09 من وجود
  `muscatWallClockToIso`/`Asia/Muscat` في نماذج المناسبات
  (`EventsPage`/`EditEventDialog`) وعروض الأسعار (`quotes.api`/`useQuotationDraft`)
  وحدود الحضور (`AttendanceClock`/`AttendancePanel`/`FaceAttendanceDialog`).

### D18 — خدمات محلية غير مستخدمة في `config.toml` — Low — DEFERRED
- لا تؤثر على العمل؛ إزالتها تجميلية وتنتظر الدورة التشغيلية التالية.

### D19 — N+1 للجاهزية — Low — DEFERRED
- مقبول عملياً لأحجام مكاتب الضيافة الحالية؛ يُعاد النظر عند ملاحظة أثر.

### D20 — احتفاظ سجل التدقيق — Low — FIXED ✅ (2026-09-09)
- **الإصلاح:** الترحيل `20260909100000_0098_audit_retention.sql` يضيف
  `public.purge_old_audit_events(org_id, older_than)` — حذف لكل منظمة على حدة،
  للمالك (OWNER) فقط (تحقق خادمي عبر `has_org_role`)، وقصّ افتراضي 24 شهراً،
  ويُسجَّل إجراء الاحتفاظ نفسه في سجل التدقيق قبل الحذف. السياسة موثقة في
  `OPERATIONS.md §5.1`.
- **التحقق:** `supabase/tests/audit_retention.test.sql` (10 تأكيدات: شكل المنح،
  رفض MANAGER، رفض مالك منظمة أخرى، دقة عدد الصفوف، إبقاء الحديث + سجل الإجراء
  نفسه، عدم المساس بمنظمات أخرى).

### D21 — ترقيم صفحات حقيقي (متابعة D3) — FIXED ✅ (2026-09-09)
- **الإصلاح:** ترقيم صفحات حقيقي 0-based لثلاث قوائم: المناسبات والعملاء
  والكتالوج — `usePagedList`/`pageRange`/`clampPage` مشتركة في
  `src/lib/pagination.ts`، ومكوّن `PaginationBar` عربي أولاً بأزرار كبيرة
  (السابق/التالي + عدّاد الصفحات ونافذة الصفوف)، وترتيب حاسم ثانوي `order by id`
  يمنع تكرار/سقوط صفوف عبر حدود الصفحات، وإعادة ضبط الصفحة عند تبديل المنظمة.
- **المتبقي (قرار منتج):** قوائم المشتريات تعتمد تجريد DataSource منفصلاً مع
  تصفية محلية — تبقى بتحذير السقف الصريح حتى يُقرَّر نمط عرضها.
- **التحقق:** `eventsPage.pagination.test.tsx` (عقد النوافذ والحدود) +
  `PaginationBar.test.tsx` (السلوك والعربية وتعطيل الاتجاهات) + تحديث محاكيات
  `EventsPage.test.tsx`.

### D22 — حفظ تلقائي لمحرر عروض الأسعار (متابعة D4) — FIXED ✅
- **الإصلاح:** حفظ مؤجل (debounce 1500ms) في `useQuotationDraft.ts` مع مرجع ثابت
  لأحدث `persistDraft` — مؤكد من الكود بتاريخ 2026-09-09، واختبارات المؤقتات
  الوهمية في `useQuotationDraft.test.tsx`.
...
## جولة التوفيق (2026-09-09) — سد الفجوات بين الجدول والتفاصيل

بعد تدقيق خارجي أشار إلى تعارضات داخل هذا السجل نفسه، أُعيد التحقق من كل بند
على حدة ضد الكود الحالي، وسُوِّيت الحالات (الجدول والتفاصيل معاً):

- **D2** — الجدول كان «FIXED (0059)» والتفاصيل «NEEDS_OWNER». الحقيقة الوحيدة:
  المخطط نظيف (0059 موجود ومُتحقق بإعادة التشغيل وpgTAP)، والمتبقي قرار تطبيق
  على قاعدة الإنتاج يخص المالك. سُجِّل الاثنان في الجدول والتفاصيل.
- **D3** — اكتمل الترقيم الحقيقي للمناسبات/العملاء/الكتالوج (D21)؛ المشتريات
  تبقى بتنبيها الصريح. حُدِّث البندان.
- **D17** — الجدول «FIXED» والتفاصيل «Deferred»: تحقق من الكود بوجود تثبيت
  Muscat في المناسبات والعروض والحضور — التفاصيل صُححت إلى FIXED مع الأدلة.
- **D20** — نُفِّذت سياسة الاحتفاظ فعلياً (الترحيل 0098 + pgTAP + توثيق
  OPERATIONS.md §5.1): لم يعد مؤجلاً.
- **D21** — لم يعد «Open»: ترقيم صفحات حقيقي بثلاث أسطح مع اختبارات العقد.
  المتبقي: قرار منتج لنمط قوائم المشتريات فقط.
- **D22** — الجدول «FIXED» والتفاصيل «Open»: الحفظ التلقائي موجود في
  `useQuotationDraft.ts` (debounce 1500ms) مع اختباراته — التفاصيل صُححت.

الأرقام الحالية المشغَّلة في هذه الجلسة (2026-09-09): typecheck 0 أخطاء ·
lint 0/0 (339 ملفاً) · **97 ملفاً / 669 اختباراً ناجحة** · build + smoke ✅ ·
قاعدة البيانات **99 ترحيلاً + كل ملفات pgTAP PASSED** عبر بوابة المستودع
المحلية (طبقة A) — البوابة الموثوقة النهائية هي مصفوفة CI على Supabase
(طبقة B).
