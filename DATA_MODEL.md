...
- `staff_members` — roster with default compensation and per-member directory
  fields (0100): hire/birth dates, nationality, job title, department,
  ...
  Members are HOSTS engaged per event (see `15-staff-attendance.md`); there is
  no salaried-employment or leave model. A tentative `staff_leaves` register
  (0100/0101) was removed by 0102.
