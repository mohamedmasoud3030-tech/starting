...
## PRE‑2 — الموقع الحي كان يقدّم كودًا أقدم من الإصلاحات (P1 نشر) — **أُغلِق**

> **حُسم بعد الدمج:** دُمج PR #55 في `main` كتجزئة `3e7089f85`، وأعاد Vercel النشر
> تلقائيًا. قراءة `version.json` بعد الدمج بنصف دقيقة:
> `{"version":"3e7089f855c5","deployedAt":"2026-09-12T22:15:02.085Z"}` — أي أن الموقع
> الحي صار يقدّم الإصلاحات الخمسة كلها، ومنها CSP الجديد. **البروفة الآن تختبر نفس
> الكود الحي**، فسقطت نقطة العمياء هذه.
> يبقى تحقق واحد لا تستطيعه هذه البيئة (لا منفذ شبكة): قراءة ترويسات الاستجابة للتأكد
> أن `img-src`/`media-src` الجديدَين يصلان فعلًا — من جهازك:
> `curl -sI https://jiwdah.vercel.app | grep -i content-security` أو DevTools → Network.

### الحالة قبل الإغلاق (للسجل)
...
| PRE‑2 | النشر | الإنتاج كان عند `5ac6cc07763c` — لا يحتوي الإصلاحات | `version.json` الحي / `PROJECT_STATUS.md:16` | **أُغلِق**: الإنتاج الآن `3e7089f855c5` (PR #55) |
