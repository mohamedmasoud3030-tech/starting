...
import { createTestSource, fullAccess, orderFixture } from "./__tests__/testDoubles";
...
describe("OrdersArea", () => {
  ...
  it("visually distinguishes every authoritative lifecycle state in Arabic", async () => {
    ...
    for (const label of ["مسودة", "معتمد", "مرسل للمورد", "مؤكد من المورد", "استلام جزئي", "تم الاستلام", "ملغي"]) {
  ...
  it("opens partial order detail with server quantities and exact OMR", async () => {
  ...
  it("keeps SENT separate from CONFIRMED and exposes both transitions", async () => {
    ...
    await waitFor(() => expect(approved.calls.send).toHaveBeenCalledWith("order-internal-APPROVED", expect.any(String)));
    unmount();
    ...
    await waitFor(() => expect(sent.calls.confirm).toHaveBeenCalledWith("order-internal-SENT", expect.any(String)));
  });

  it.each([
    ["CONFIRMED", false],
    ["RECEIVED", true],
  ] as const)("uses adapter receiving capability in %s", async (status, receiveDisabled) => {
  ...
  it("does not render commercial amounts without server-derived permission", async () => {
    const controls = createTestSource({ orders: [orderFixture("DRAFT")] });
  ...
  it("creates an exact S5A consumable draft using a tracked catalog item", async () => {
    ...
    await user.type(screen.getByLabelText(/سعر الوحدة المتفق عليه/), "1.001");
