    ...
    <Card className="p-3 sm:p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-xs font-semibold leading-tight text-slate-500 sm:text-sm">{label}</p>
          <p className="mt-1 text-2xl font-bold tabular-nums text-slate-900">{value ?? "—"}</p>
        </div>
        {Icon && (
          <div
            className={`hidden h-10 w-10 shrink-0 sm:flex items-center justify-center rounded-xl ${toneClasses[tone]}`}
          >
