-- 0104: accounting aging/statement RPCs must be VOLATILE.
--
-- These five functions were declared STABLE but call
-- public.ensure_system_chart(), which INSERTs the system chart on first use.
-- PostgreSQL rejects writes inside a STABLE function
-- ("cannot execute INSERT in a read-only transaction" / 25006), which is what
-- the «تعذّر تحميل الأعمار» error on the accounting page is. Bodies are
-- unchanged; only the volatility marker differs. Grants are preserved
