-- ============================================================================
-- 0048 — R10 canonical command idempotency consolidation
--
...
-- Preserve any already-recorded replay facts before the old physical tables
-- disappear. The scope is part of the key to preserve the former per-domain
-- uniqueness semantics exactly.
...
-- Existing domain helpers remain as stable RPC-internal adapters. Application
-- commands therefore need no behavioral rewrite in this migration.
...
-- Read-only compatibility names keep existing diagnostics/tests valid without
-- reintroducing duplicate storage. They are intentionally not client contracts.
create view public.procurement_command_idempotency as
...
create view public.payments_command_idempotency as
select organization_id, idempotency_key, command_name, request_fingerprint,
       result_entity, result_id, response_payload, actor_id, created_at
  from public.command_idempotency
 where command_scope = 'PAYMENTS';

create view public.staff_payroll_command_idempotency as
select organization_id, idempotency_key, command_name, request_fingerprint,
       result_entity, result_id, response_payload, actor_id, created_at
  from public.command_idempotency
