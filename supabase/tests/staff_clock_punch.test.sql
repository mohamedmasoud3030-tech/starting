-- ============================================================================
-- 0073 — operational clock punch (دخول / خروج)
-- ============================================================================
begin;
select plan(10);
...
select lives_ok($$select public.clock_staff_in('98000000-0000-0000-0000-0000000000a1','98000000-0000-0000-0000-0000000000e1','98000000-0000-0000-0000-0000000000f1','98000000-0000-0000-0000-0000000000d1','MORNING',null,'98200000-0000-0000-0000-000000000001')$$,'OWNER clocks a host in');
select is((select earned_amount::text from public.staff_attendance where event_id='98000000-0000-0000-0000-0000000000e1' and status<>'VOIDED'),'0.000','open punch keeps earned at 0.000 OMR');
select is((select check_out is null from public.staff_attendance where event_id='98000000-0000-0000-0000-0000000000e1' and status<>'VOIDED'),true,'open punch has no check-out yet');

select throws_ok($$select public.clock_staff_in('98000000-0000-0000-0000-0000000000a1','98000000-0000-0000-0000-0000000000e1','98000000-0000-0000-0000-0000000000f1','98000000-0000-0000-0000-0000000000d1','MORNING',null,'98200000-0000-0000-0000-000000000002')$$,'23505','ATTENDANCE_SLOT_ALREADY_RECORDED','double clock-in of the same slot is rejected');

set local "request.jwt.claims"='{"sub":"98000000-0000-0000-0000-0000000000b2","role":"authenticated"}';
select throws_ok($$select public.clock_staff_in('98000000-0000-0000-0000-0000000000a1','98000000-0000-0000-0000-0000000000e1','98000000-0000-0000-0000-0000000000f1','98000000-0000-0000-0000-0000000000d1','EVENING',null,'98200000-0000-0000-0000-000000000003')$$,'42501','NOT_AUTHORIZED','WAREHOUSE cannot punch the clock');

set local "request.jwt.claims"='{"sub":"98000000-0000-0000-0000-0000000000b1","role":"authenticated"}';
select throws_ok($$select public.clock_staff_in('98000000-0000-0000-0000-0000000000a1','98000000-0000-0000-0000-0000000000e1','98000000-0000-0000-0000-0000000000f2',null,'MORNING',null,'98200000-0000-0000-0000-000000000004')$$,'P0002','ASSIGNMENT_NOT_FOUND','unassigned host cannot clock in');
...
select lives_ok($$select public.clock_staff_out('98000000-0000-0000-0000-0000000000a1','98000000-0000-0000-0000-0000000000e1','98000000-0000-0000-0000-0000000000f1',null,'98200000-0000-0000-0000-000000000005')$$,'OWNER clocks the host out');
select is((select earned_amount::text from public.staff_attendance where event_id='98000000-0000-0000-0000-0000000000e1' and status<>'VOIDED'),'11.000','closed punch earns 5.5h * 2.000 = 11.000 OMR');
select is((select hours_worked::text from public.staff_attendance where event_id='98000000-0000-0000-0000-0000000000e1' and status<>'VOIDED'),'5.500','closed punch stores 5.500 hours');

select throws_ok($$select public.clock_staff_out('98000000-0000-0000-0000-0000000000a1','98000000-0000-0000-0000-0000000000e1','98000000-0000-0000-0000-0000000000f1',null,'98200000-0000-0000-0000-000000000006')$$,'P0001','CLOCK_IN_REQUIRED','checkout without an open punch fails');
